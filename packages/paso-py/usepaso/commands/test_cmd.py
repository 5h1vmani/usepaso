...
def register(cli_group):
    ...
    def test_cmd(capability, file, env_file, param, dry_run, all_caps, timeout):
        """Test a capability against the live API (or --dry-run, minus the consequences)."""
        try:
            _run_test(capability, file, env_file, param, dry_run, all_caps, timeout)
        except SystemExit:
            raise
...
def _run_test(capability, file, env_file, param, dry_run, all_caps, timeout):
