...
│   │   │   │   ├── serve.ts
│   │   │   │   ├── doctor.ts
│   │   │   │   └── connect.ts    # connect/disconnect to MCP clients (Claude Desktop, Cursor, etc.)
│   │   │   ├── utils/
...
│   │   │   │   └── env.ts       # .env loading, git tracking, gitignore management
...
│       │   │   ├── serve_cmd.py
│       │   │   ├── doctor_cmd.py
│       │   │   └── connect_cmd.py  # connect/disconnect to MCP clients
│       │   ├── utils/
...
│       │   │   └── env.py        # .env loading, git tracking, gitignore management
│       │   ├── parser.py
│       │   ├── validator.py
│       │   ├── executor.py
│       │   ├── generators/
...
- `connect`/`disconnect`: Same output messages (note: JS config uses `npx`, Python uses `usepaso` binary)
...
All CLI output, error messages, help strings, and READMEs follow these rules. The full brand voice guide lives at `usepaso.dev/brand/voice-and-tone.md`.

### Core Voice

Principled, calm, technical, warm, direct. Like a senior engineer who reads the RFC before writing the blog post.
...
- Perspective: 1st plural for conviction ("we believe"), 2nd person for DX ("you declare," "your service"). Never 3rd-person corporate.

### Punctuation

- Periods over exclamation marks. Always.
- No em dashes. Use a comma, a period, or restructure the sentence.
- Colons to set up specifics: like this.
- No ellipsis (...) unless in CLI output.
- Oxford comma: yes.

### Error Messages

- Say what's wrong (plain language).
- Say what to do (next step).
- Link to docs (if applicable).
- No emoji. No "oops." No jargon codes without explanation.

### Words We Own

"Declare" (not "define" or "configure"), "agent-ready," "ship" (not "deploy" in casual contexts), "sovereignty," "protocol-agnostic," "production-grade."

### Words and Phrases to Avoid

These signal hype, corporate filler, or AI-generated text. If any appear in paso copy, rewrite the sentence.

**Words:** "revolutionary," "game-changing," "seamless," "AI-powered," "leverage," "utilize," "best-in-class," "disrupt," "excited," "empower," "solution," "cutting-edge," "world-class."

**Phrases:** "In today's fast-paced world," "Let's dive in," "Unlock your potential," "At the end of the day," "It goes without saying," "Moving forward," "Synergy," "It's worth noting that," "It's important to remember," "I'm excited to share," "Moreover / Furthermore / Additionally / In conclusion," "That being said," "Ecosystem," "Paradigm shift," "Firstly... secondly... finally..."

If you need a transition, use a short sentence or a line break. If you need emphasis, use a fact.
